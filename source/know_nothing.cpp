...
bool knownothing::check_integrity(vector<uint8_t> input)
{
    ...
    for (uint32_t index = 0; index < messages_number; index++)
    {
        ...
        uint32_t message_size = deserialize({input[current_position++],
    ...
    // validates if the messages numbers and the messages with their sizes
    // matches with the input size
