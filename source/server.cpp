...
ClientSocket::ClientSocket(const int32_t file_descriptor,
                           const struct sockaddr_in address)
    : Socket(file_descriptor, address) {
  // available to READ | Requests edge-triggered notification | available WRITE
  // | peer shutdow edge-triggered notification |
...
vector<uint8_t> ClientSocket::get() const {
  ...
  do {
    ...
    // TODO: Since vector is backed by an array, we need to improve the number
    // of times that realloc is done under the hood to be more effective
    // Maybe(?) https://cplusplus.com/reference/vector/vector/reserve/

    copy(buffer, buffer + bytes_read, back_inserter(vector));

    // We request for N bytes, returned N - 1, that means that we have nothing
    // else to read from the file descriptor
...
Server::Server(const uint16_t port, const uint16_t pending_connections,
               Dispatcher &dispatcher)
    : port(port), pending_connections(pending_connections),
      socket(build_server_socket(port)), dispatcher(dispatcher) {}
...
void Server::start() {
  ...
  do {
    for (const auto &event : epoll.wait_for_events()) {
      if (!running) {
        // while waiting for events, or processing some event, the server
        // requested to stop
        break;
      }
...
void Server::stop() {
  running = false;
  cout << "Request to stop the server accepted! Gracefully stopping the server!"
       << endl;
}

void Server::handle_new_connection() {
  unique_ptr<const ClientSocket> client(socket.accept_connection());
  current_connections.insert(make_pair(client->file_descriptor, move(client)));
}
...
EpollHandler::~EpollHandler() { close(file_descriptor); }
...
bool EpollHandler::add(int32_t file_descriptor, uint32_t events) const {
  ...
  return epoll_ctl(this->file_descriptor, EPOLL_CTL_ADD, file_descriptor,
                   &event);
}

bool EpollHandler::del(int32_t file_descriptor) const {
  return epoll_ctl(this->file_descriptor, EPOLL_CTL_DEL, file_descriptor,
                   nullptr);
}
...
const vector<struct epoll_event> EpollHandler::wait_for_events() const {
  ...
  auto total =
      epoll_wait(file_descriptor, events_buffer,
                 sizeof(events_buffer) / sizeof(events_buffer[0]), timeout);
  if (total < 0) {
